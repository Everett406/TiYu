...
| `vibrate_on_correct` | bool | 答对轻震反馈（刷题/特训，考试不受影响） | true |
...
| v2.11.1 | 43 | **新功能：系统「其他应用打开」导入题库**——微信/文件管理器对题库 ZIP/CSV 点「从其他应用打开」直达导入预览（内容嗅探判格式、128MB 上限），确认即导入并自动切换当前题库，可一键开始刷题；**撤销 v2.11.0 随机选项**（答案解析按固定选项字母书写，打乱后解析错位）；DB 保持 v5，从 v2.10.3/v2.11.0 升级均无损 |
| v2.11.2 | 44 | **关闭特效改为「亚克力模糊」**：果冻 goo 动效整体移除（底栏拖尾/按钮液泡/滑杆开关融合/弹窗过冲），关闭特效=干净毛玻璃+正常手感；液态玻璃模式零改动；设置页文案同步；static_check goo 残留禁令 |
| v2.11.3 | 45 | **新功能：答对震动**——刷题/特训即时判定答对时轻震一下（平台标准 CLICK 触感，简答自评答对同享，答错不震）；设置页「刷题」区可开关（默认开）；考试为整卷提交不接入；static_check 新增 3.11 节防回归 |
