...
v2.13.0 实测反馈：匹配率已上来（都能搜到），残留漏题/大框；用户提出单题工作流
（默认单体单体搜 + 手动划定题目区域 + 支持横拍），本轮实现：

- **单题模式（默认）**：识别完成后自动框住一道题（优先第一个命中的题块），
  下方只出这一张结果卡（常展开）；「上一题 / 下一题」在自动题块间切换计数；
  右上「整页」随时切回 v2.13 全部框选 + 列表视图。
- **手指拖画框**：单题模式下在图上直接拖动画框（detectDragGestures，touch slop
  与点块点击自然区分互不干扰），松手 → `linesInRegion`（行与区域相交 ≥40% 面积）
  提取框内 OCR 行 → `segmentQuestions` 重切 → 复用缓存题库即时匹配；手框兜底
  一切自动切题的漏题/连体。框太小（<30px）忽略；框到多题时全部展示可再清除。
- 非选中自动块淡显（1dp 40% 透明度、去角标），选中块/手框 accent 高亮——
  视觉焦点集中到当前题。
- **取景框横向长条**：高/宽 0.62 → 0.45（一道题的典型比例，单题导向）；
  提示「对准一道题放进框内 · 横握手机也可以，会自动摆正」（Activity 仍锁
  portrait，横握仅影响握持姿势，CameraX 拍照 EXIF 自动摆正，v2.13 已支持）。
- PhotoSearch.kt 新增 `linesInRegion`；static_check 新增 3.15 节（单题默认开启/
  手框链路/上下题导航/取景框 0.45 比例防回退）。

## [2.13.0] - 2026-09-16

### 拍照搜题大升级 —— 自建相机页 + 斜拍自动矫正 + 切题/匹配重写（第四十六轮）

用户实测反馈四个痛点：框选不一题一框（连体大框）、部分题搜不到、担心题号被
OCR 错识/漏识、希望支持斜拍并自建拍照界面。本轮逐一解决：

- **自建相机页（CameraX）**：搜索页相机按钮直连应用内相机——取景框引导
  （框外压暗 + 白色四角线）、快门/闪光/相册一步直达；相机权限运行时申请，
  拒绝后仍有相册出路（功能不丢失）。弃用系统相机 TakePicture 链路；
  manifest 新增 CAMERA 权限 + uses-feature camera.any（required=false）。
- **斜拍自动矫正（deskew）**：首遍 OCR 用各行基线角（cornerPoints tl→tr 向量，
  行宽达行高 3 倍才可信）取中位数测倾斜，|角度| ≥1.2° 且 ≤12° 时旋转转正后
  重识别；>12° 多为透视形变不硬转。成本：斜拍时多一遍 OCR（约 1~5 秒）。
- **切题 v2（题号不再是唯一判据）**：①题号回落重开——当前块选项已齐（≥3）
  时题号行无论号值都开新题（救题号错识偏大后全页连体、双栏号序跳变）；
  ②漏号结构重开——选项齐（单/多选 ≥4 或判断式 2 短选项）又来 ≥12 字长行
  → 强拆开新题（救题号漏识连体）；③页码/页脚行整行丢弃（纯数字装饰式
  「- 3 -」与「第 x 页」式，不再污染题干稀释匹配）。
- **匹配 v2（不上向量模型的论证：用户场景是 OCR 文字 vs 题库原文的同源匹配，
  字符级对齐理论即最优，embedding 优势在措辞不同的语义检索）**：段级匹配——
  题干整串 + 每个原始行（≥6 字）+ 长串滑窗（40 字窗/20 字步）逐段对题库求分
  取最大（行级天然隔离连体块内不同题的文本）；Dice 相似度为主分，包含率
  （|A∩B|/min，0.94× 降权且仅 ≥10 字段启用）为辅对抗截断；选项串兜底 ×0.9 保留。
- 新增依赖 androidx.camera ×4（camera-core/camera2/lifecycle/view 1.4.2）。
- static_check 3.14 重写：CAMERA 断言反转为「必须声明」+ uses-feature 断言 +
  CameraX 依赖断言 + 矫正/段级匹配/页脚丢弃/连体强拆 关键符号防回退。
...
- 用户实测反馈：顺序刷到 57/800 后跳做第 800 题，答完停在末题不动（既无下一题、
  也不跳回第一个没刷的题）；退出重进后总题数 800→743、从第 1 题开始，
  观感为「57 题记录丢了」。
- 根因：v2.8.6 循环补漏的触发时机在「重进」而非当场——末题提交后翻页条件
  `currentPage < size-1` 不成立，就地停留；重进时 `sessionAtEnd` 判定本轮结束，
  新开补漏轮只装未刷过的题（800−已答≈743），且补漏轮会话从 index 0 开始、
  序号显示为会话内序号，叠加造成「进度归零/题变少」的错觉。作答记录实际
  全部在 DB（错题本/统计/首页进度无损），被覆盖的只是会话快照载体。
- 修复：①答对且自动切题开 → 在末题时自动跳到本轮第一个未答的题（target 提交
  时刻预计算，delay 期间手动翻页不被覆盖）；②答错停在末题看解析后，手动
  「下一题」同样跳到第一个未答的题（本轮全刷完则停住不动）；③补漏轮副标题
  新增「· 补漏轮 · 此前已刷 N 题」口径标识（roundCovered 非空判定），总题数
  变化不再被误解。
- 边界保持：补漏轮机制本身不变（退出重进仍补漏）；随机小练/错题特训不受影响；
  简答题不触发自动切题（与既有口径一致）。
- static_check 新增 3.13 节：末题续轮逻辑（2 处调用点）+ 补漏标识 + 旧条件回退禁令。
...
- 刷题/错题特训即时判定「回答正确」时轻震一下：采用平台标准 CLICK 预设触感
  （跟随系统触感强度设置，各家 ROM 出厂调校，不易过重或过轻）；答错不震。
- 设置页「刷题」区新增开关「**答对震动**」，默认开启，副标题注明考试不受影响。
- 考试/模考不接入：整卷提交、交卷出分才有对错，没有逐题即时判定点。
- 简答题自评「答对了」同样轻震（与其它题型答对口径一致）；幂等保护防重复触发。
- 实现：`util/VibrateFeedback.kt`（VibratorManager + createPredefined(EFFECT_CLICK)，
  minSdk 31 无旧版分支；hasVibrator 防护；异常静默不影响答题记账）；唯一调用点
  `PracticeScreen.onCommit` 正确性首次判出处（刷题/特训/随机小练共用）；开关按
  本页规约只订阅单字段（无关 settings 写入不穿透刷题页）；static_check must
  断言 +3、新增 3.11 节锁权限/工具/调用点并禁止考试路径接入。
