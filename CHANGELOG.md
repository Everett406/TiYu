...
### 新增 —— 随机选项（第四十轮：用户需求，刷题 + 模考可开关、错题本保持原序）

- **刷题随机选项**：配置页「题目顺序」卡新增开关；开刷后选择题选项乱序，
  每轮重排（快照保存会话盐，接续恢复顺序不变；重新开始重新洗）。
- **模考随机选项**：配置页「高级选项」新增开关（随配置持久化）；开考生成
  会话盐随 exam_records 落库（DB v4→v5，老库无损迁移），进程重启 resumeExam
  依盐还原开考时顺序；快速模考沿用上次配置的开关。
- **答案映射防线（本版本核心命题：打乱后答案绝不受影响）**：
  - 新增 `data/repo/OptionShuffle.kt`：视图层乱序（perm[显示]=原始 双射）+
    数据层恒为原始空间——作答在 onCommit/onAnswer 经 `uaToOriginal` 换算后
    才判分/落库，`judgeAnswer`、错题本、统计零改动；
  - 已答高亮经 `uaToDisplay` 回显，与 [uaToOriginal] 严格互逆；
  - salt=0 恒等于「不打乱」：旧快照/旧模考记录/关开关全兼容；
  - 判断题（√/× 位置语义）与填空/简答不参与打乱；错题特训（src=wrong）
    恒不生成盐（用户口径：错题本保持原始顺序）。
- **OptionShuffleTest（JVM 单测）**：双射性 200 组；同盐稳定性；单选 500 题
  ×穷举点选位置、多选 500 题×穷举勾选子集——显示空间判定与原始空间判定
  全等；端到端 300 轮「选对内容必判对/选错内容必判错」；salt=0 与不适用
  题型透传；newSalt 恒非零。
- static_check 新增 3.8 断言：OptionShuffle 四核心符号、两链路 uaToOriginal
  换算、错题特训守卫、单测文件存在性，误删即 FAIL。
- app 增加 testImplementation junit；`:app:testDebugUnitTest` 随本地验证执行。
